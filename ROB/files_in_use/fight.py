...
bob = pygame.image.load('images/sprites/bob/walking_right_green_0.png').convert_alpha()
...
def player_right_pics(self, match):
    ...
    if match == 0:
        self.images = []
        for i in range(1, 3):
            img = pygame.image.load(f'images/sprites/bob/walking_right_green_{i}.png').convert_alpha()
            self.images.append(img)
            self.image = self.images[0]
            self.rect = self.image.get_rect()
